...
export default function CompanyProperties() {
  ...
  const regularProps = trpc.properties.list.useQuery(undefined, { enabled: !isViewingAsCompany });
  const viewAsProps = trpc.adminViewAs.companyProperties.useQuery(
    { companyId: viewAs.companyId! },
    { enabled: !!isViewingAsCompany }
  );

  const properties = isViewingAsCompany ? viewAsProps.data : regularProps.data;
  const isLoading = isViewingAsCompany ? viewAsProps.isLoading : regularProps.isLoading;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {isViewingAsCompany ? `Viewing properties for ${viewAs.companyName}` : "Manage your property portfolio"}
          </p>
        </div>
        {canCreate && (
        ...
        <Card className="bg-card border-border">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-muted-foreground">
              {isViewingAsCompany ? "This company has no properties yet." : "No properties yet. Add your first property to start creating maintenance jobs."}
            </p>
          </CardContent>
        </Card>
      ) : (
