...
import { Navigation2, Clock, MapPin, User, RefreshCw, WifiOff, Timer, DollarSign, Route, History } from "lucide-react";
...
/** Calculate live running cost from clock-in time and hourly rate */
function calcLiveCost(clockInTime: number, hourlyRate: string | null): string {
  if (!hourlyRate) return "—";
  const rate = parseFloat(hourlyRate);
  if (!rate) return "—";
  const elapsedHours = (Date.now() - clockInTime) / 3_600_000;
  return (rate * elapsedHours).toFixed(2);
}

/** Calculate final cost from totalMinutes and hourlyRate */
function calcFinalCost(totalMinutes: number | null, hourlyRate: string | null): string {
  if (!totalMinutes || !hourlyRate) return "—";
  const rate = parseFloat(hourlyRate);
  if (!rate) return "—";
  return ((rate / 60) * totalMinutes).toFixed(2);
}

/** A component that re-renders every second to show a live cost */
function LiveCost({ clockInTime, hourlyRate, className }: { clockInTime: number; hourlyRate: string | null; className?: string }) {
  const [cost, setCost] = useState(() => calcLiveCost(clockInTime, hourlyRate));
  useEffect(() => {
    if (!hourlyRate) return;
    const id = setInterval(() => setCost(calcLiveCost(clockInTime, hourlyRate)), 1000);
    return () => clearInterval(id);
  }, [clockInTime, hourlyRate]);
  if (!hourlyRate) return null;
  return <span className={className}>Job cost: ${cost}</span>;
}

/** A component that re-renders every second to show elapsed time */
function LiveElapsed({ clockInTime, className }: { clockInTime: number; className?: string }) {
...
export default function LiveTracking() {
  ...
  const [replaySession, setReplaySession] = useState<{ id: number; title: string } | null>(null);
  ...
  // Past sessions
  const { data: pastSessions, isLoading: pastLoading } = trpc.timeTracking.getCompletedSessionsForCompany.useQuery({ limit: 50 }, {
  ...
  const calculateEta = useCallback((session: ActiveSession) => {
    ...
    directionsServiceRef.current.route(
      { origin: { lat, lng }, destination: { lat: jobLat, lng: jobLng }, travelMode: google.maps.TravelMode.DRIVING },
      (result, status) => {
        if (status === google.maps.DirectionsStatus.OK && result?.routes?.[0]?.legs?.[0]) {
          const leg = result.routes[0].legs[0];
          setEtaMap(prev => new Map(prev).set(session.sessionId, { text: leg.duration?.text ?? "Unknown", duration: leg.distance?.text ?? "" }));
        } else {
  ...
  const updateMarkers = useCallback((map: google.maps.Map, data: ActiveSession[]) => {
    ...
    markersRef.current.forEach((marker, id) => { if (!sessionIds.has(id)) { marker.map = null; markersRef.current.delete(id); } });
    jobMarkersRef.current.forEach((marker, id) => { if (!sessionIds.has(id)) { marker.map = null; jobMarkersRef.current.delete(id); } });
    ...
    data.forEach((session) => {
      ...
      if (markersRef.current.has(session.sessionId)) {
        ...
        el.style.cssText = `width:36px;height:36px;border-radius:50%;background-color:${isStale ? "#6b7280" : "#3b82f6"};border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;cursor:pointer;`;
        ...
        const marker = new google.maps.marker.AdvancedMarkerElement({ map, position: contractorPos, title: session.contractorName ?? "Contractor", content: el });
        marker.addListener("click", () => { setSelectedSessionId(session.sessionId); calculateEta(session); });
      ...
      if (jobLat && jobLng && !jobMarkersRef.current.has(session.sessionId)) {
        const el = document.createElement("div");
        el.style.cssText = `width:28px;height:28px;border-radius:4px;background-color:#f59e0b;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;`;
        ...
        const jobMarker = new google.maps.marker.AdvancedMarkerElement({ map, position: { lat: jobLat, lng: jobLng }, title: session.jobTitle ?? "Job Site", content: el });
  ...
  const handleMapReady = useCallback((map: google.maps.Map) => {
    ...
    directionsRendererRef.current = new google.maps.DirectionsRenderer({
      ...
      polylineOptions: { strokeColor: "#3b82f6", strokeOpacity: 0.6, strokeWeight: 3 },
    });
    if (sessions) updateMarkers(map, sessions as ActiveSession[]);
  }, [sessions, updateMarkers]);

  useEffect(() => {
    if (mapRef.current && sessions) updateMarkers(mapRef.current, sessions as ActiveSession[]);
  }, [sessions, updateMarkers]);
  ...
  useEffect(() => {
    if (!selectedSessionId && directionsRendererRef.current) directionsRendererRef.current.setDirections({ routes: [] } as any);
  }, [selectedSessionId]);
    ...
    <div className="space-y-4 h-full">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Live GPS positions and job history</p>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-xs text-muted-foreground">Live tab refreshes every 5s · Last: {formatLastSeen(lastRefresh)}</span>
      ...
      <Tabs defaultValue="live" className="w-full">
        <TabsList className="mb-4">
          <TabsTrigger value="live" className="gap-2">
            <Navigation2 className="h-4 w-4" /> Live Jobs
            {sessions && sessions.length > 0 && (
              <Badge className="ml-1 h-5 px-1.5 text-[10px] bg-blue-500 text-white">{sessions.length}</Badge>
            )}
          </TabsTrigger>
          <TabsTrigger value="past" className="gap-2">
            <History className="h-4 w-4" /> Past Jobs
          </TabsTrigger>
        </TabsList>

        {/* ── LIVE JOBS TAB ──────────────────────────────────────────────── */}
        <TabsContent value="live">
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-4" style={{ height: "calc(100vh - 280px)", minHeight: 500 }}>
            {/* Left panel */}
                    ...
                    <Card
                      ...
                      onClick={() => {
                        ...
                        if (!isSelected && mapRef.current) {
                          const lat = parseFloat(session.latestLat ?? session.clockInLat ?? "0");
                          const lng = parseFloat(session.latestLng ?? session.clockInLng ?? "0");
                          if (lat && lng) { mapRef.current.panTo({ lat, lng }); mapRef.current.setZoom(14); }
                        }
                      }}
                    >
                      <CardContent className="p-4 space-y-2">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <span className="font-medium text-sm text-card-foreground truncate">{session.contractorName ?? "Contractor"}</span>
                          </div>
                        ...
                        {session.jobTitle && (
                          <p className="text-xs text-muted-foreground truncate flex items-center gap-1">
                            <MapPin className="h-3 w-3 shrink-0" />{session.jobTitle}
                          </p>
                        )}
                        {session.jobAddress && <p className="text-xs text-muted-foreground truncate pl-4">{session.jobAddress}</p>}

                        {/* Live cost */}
                        {session.hourlyRate && (
                          <div className="flex items-center gap-1.5 text-xs">
                            <DollarSign className="h-3 w-3 text-green-400 shrink-0" />
                            <LiveCost clockInTime={session.clockInTime} hourlyRate={session.hourlyRate} className="text-green-400 font-medium" />
                          </div>
                        )}
                        ...
                        {isSelected && (
                          <div className="flex items-center gap-1.5 text-xs">
                            ...
                            {eta === "loading" ? <span className="text-muted-foreground">Calculating ETA…</span>
                              : eta ? <span className="text-amber-400 font-medium">ETA: {eta.text}{eta.duration ? ` · ${eta.duration}` : ""}</span>
                        ...
                        <div className="flex items-center justify-between text-xs text-muted-foreground pt-1">
                          <span className="flex items-center gap-1">
                            ...
                            On clock: <LiveElapsed clockInTime={session.clockInTime} />
                          </span>
            ...
            <div className="lg:col-span-3 rounded-xl overflow-hidden border border-border relative">
              <MapView className="w-full h-full" initialCenter={{ lat: 39.8283, lng: -98.5795 }} initialZoom={4} onMapReady={handleMapReady} />

              {/* Selected contractor overlay */}
              {selectedSession && (
                <div className="absolute bottom-4 left-4 right-4 max-w-sm bg-background/95 backdrop-blur border border-border rounded-xl p-4 shadow-xl">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {selectedSession.contractorPhone && <p className="text-xs text-muted-foreground">{selectedSession.contractorPhone}</p>}
                      </div>
                    </div>
                    <button onClick={() => setSelectedSessionId(null)} className="text-muted-foreground hover:text-foreground text-lg leading-none">×</button>
                  </div>
                  <div className="mt-3 space-y-1.5 text-xs text-muted-foreground">
                    ...
                    {selectedSession.jobAddress && <p className="pl-5">{selectedSession.jobAddress}</p>}

                    {/* Live cost in overlay */}
                    {selectedSession.hourlyRate && (
                      <div className="flex items-center gap-1.5 border-t border-border pt-2 mt-1">
                        ...
                        <LiveCost clockInTime={selectedSession.clockInTime} hourlyRate={selectedSession.hourlyRate} className="text-green-400 font-semibold text-sm" />
                        <span className="text-muted-foreground ml-1">@ ${selectedSession.hourlyRate}/hr</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {selectedEta === "loading" ? <span>Calculating ETA…</span>
                        : selectedEta ? <span className="text-amber-400 font-medium">ETA to job site: {selectedEta.text}{selectedEta.duration && <span className="text-muted-foreground font-normal"> · {selectedEta.duration}</span>}</span>
                        : <span>ETA unavailable</span>}
                    </div>

                    <p className="flex items-center gap-1.5 pt-1">
                      <Clock className="h-3.5 w-3.5 shrink-0" />
                      On clock for <LiveElapsed clockInTime={selectedSession.clockInTime} className="ml-0.5" />
            ...
            {pastLoading ? (
              ...
              <Card className="bg-card border-border">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Completed contractor sessions will appear here.</p>
                </CardContent>
              </Card>
            ) : (
              (pastSessions as CompletedSession[]).map((session) => {
                const finalCost = calcFinalCost(session.totalMinutes, session.hourlyRate);
                const clockOutMethodLabel: Record<string, string> = {
                  manual: "Manual clock-out",
                  auto_geofence: "Auto (returned to origin)",
                  auto_timeout: "Auto (timeout)",
                  admin: "Admin clock-out",
                };
                return (
                  <Card key={session.sessionId} className="bg-card border-border hover:border-primary/30 transition-colors">
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between gap-4">
                        <div className="min-w-0 flex-1 space-y-1.5">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-card-foreground truncate">{session.contractorName ?? "Contractor"}</span>
                            {session.isEmergency && <Badge variant="destructive" className="text-[10px]">Emergency</Badge>}
                            {session.jobStatus && (
                              <Badge variant="outline" className="text-[10px] capitalize">{session.jobStatus.replace("_", " ")}</Badge>
                            )}
                          </div>
                          {session.jobTitle && (
                            <p className="text-sm text-muted-foreground flex items-center gap-1">
                              <MapPin className="h-3.5 w-3.5 shrink-0" />{session.jobTitle}
                            </p>
                          )}
                          {session.jobAddress && <p className="text-xs text-muted-foreground pl-5">{session.jobAddress}</p>}
                          <div className="flex items-center gap-4 text-xs text-muted-foreground flex-wrap pt-1">
                            <span className="flex items-center gap-1">
                              <Clock className="h-3 w-3" /> {formatDuration(session.totalMinutes)}
                            </span>
                            {session.hourlyRate && <span className="text-muted-foreground">${session.hourlyRate}/hr</span>}
                            {session.clockOutMethod && (
                              <span className="text-muted-foreground">{clockOutMethodLabel[session.clockOutMethod] ?? session.clockOutMethod}</span>
                            )}
                            {session.clockOutTime && (
                              <span>{new Date(session.clockOutTime).toLocaleDateString()} {new Date(session.clockOutTime).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                            )}
                          </div>
                        </div>
      ...
      {replaySession && (
        <RouteReplayDialog
          ...
          jobId={replaySession.id}
          jobTitle={replaySession.title}
        />
      )}
    </div>
  );
